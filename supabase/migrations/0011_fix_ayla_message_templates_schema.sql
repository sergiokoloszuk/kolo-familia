-- ============================================================
-- Kolo Família — Migração 0011
--   Corrige o schema de public.ayla_message_templates.
--
--   Uma migração anterior (não-canônica) criou a tabela com colunas
--   incompatíveis com o código: id/slug/canal/conteudo em vez de
--   key/category/variations. Isso é um DROP + recriação limpa com o
--   schema esperado por src/lib/ayla/messageTemplates.ts.
--
--   Os 13 templates iniciais (com variations múltiplas, placeholders
--   {nomeMae}/{nomeMembro}, variables e versao) são recriados a partir
--   do código de produção. Edições posteriores devem ser feitas via
--   /admin/mensagens.
-- ============================================================

drop table if exists public.ayla_message_templates cascade;

create table public.ayla_message_templates (
...
-- Força reload do schema cache do PostgREST (importante quando rodando via psql direto,
-- bypassando o Studio que faz isso automaticamente).

...
export function chunkificar(texto, meta) {
  ...
  // Markdown da Pós: cabeçalhos delimitam temas/faixas; a bibliografia não
  // vira conhecimento. O corpo original é preservado, inclusive ressalvas.
  const markdownPos = /^# Compêndio de Neurodesenvolvimento:/m.test(texto);
  let faixaMarkdown = null;
  if (markdownPos) {
    let titulo = null;
    let corpo = [];
    let nucleo = "fundamentos";
    const fecharMarkdown = () => {
      if (titulo && corpo.join("\n").trim()) fatias.push({
        nucleo, secao: titulo, contexto: null,
        texto: corpo.join("\n").trim(), faixa: faixaMarkdown,
      });
      corpo = [];
    };
    for (const linha of linhas) {
      if (/^# PARTE 3\b/.test(linha)) { fecharMarkdown(); titulo = null; break; }
      if (/^#{1,3}\s/.test(linha)) {
        fecharMarkdown();
        titulo = null;
        if (!/^###\s|^## .*DIRETRIZ/.test(linha)) continue;
        titulo = linha.replace(/^#+\s*/, "");
        nucleo = /Comunicação|Fala:/.test(titulo) ? "comunicacao"
          : /Sensorial/.test(titulo) ? "sensorial"
          : /Autocuidado|Vida Adulta/.test(titulo) ? "autonomia"
          : /Relações Sociais/.test(titulo) ? "socializacao"
          : /Idade Escolar/.test(titulo) ? "aprendizagem"
          : /Adolescência/.test(titulo) ? "socializacao" : "fundamentos";
        const meses = titulo.match(/\((\d+) a (\d+) meses/);
        const anos = titulo.match(/\((\d+) a (\d+) anos/);
        faixaMarkdown = /Vida Adulta/.test(titulo) ? {min:216,max:null,rotulo:"18 anos ou mais"}
          : meses ? {min:+meses[1],max:+meses[2],rotulo:`${meses[1]}-${meses[2]} meses`}
          : anos ? {min:+anos[1]*12,max:+anos[2]*12,rotulo:`${anos[1]}-${anos[2]} anos`}
          : {min:null,max:null,rotulo:null};
      } else if (titulo && !/^---\s*$/.test(linha)) corpo.push(linha);
    }
    if (titulo) fecharMarkdown();
  }

  for (const linha of markdownPos ? [] : linhas) {
  ...
  const empurrar = (fatia, texto_original, tipo, extra = {}) => {
    ...
    const faixa = fatia.faixa ?? detectarFaixa(`${fatia.secao ?? ""} ${t}`);

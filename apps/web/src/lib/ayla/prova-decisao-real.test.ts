...
describe.skipIf(!process.env.PROVA_REAL)("decisao", () => {
  it("devo chamar a amiga?", async () => {
    const R: unknown[] = [];
    const menina = { nome: "Manu", genero: "feminino" };
    const m = familia(
      { sensorial: "sente bastante barulho e lugar cheio", desafios_regulacao: "fica muito frustrada quando e contrariada" },
      { gostos: "adora banana, cavalos e desenhar", socializacao: "brinca melhor com uma crianca de cada vez" },
      menina,
    );
    R.push(await turno(m, "Minha filha brigou com a amiga e agora quer levar para ela uma banana.", "1 banana"));
    R.push(await turno(m, "Devo chamar ela para vir em casa e assistir um filme?", "2 decisao"));

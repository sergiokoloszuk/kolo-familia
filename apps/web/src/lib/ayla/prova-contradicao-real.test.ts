...
describe.skipIf(!process.env.PROVA_REAL)("contradicao perfil x relato — quatro dominios", () => {
  it("fala, alimentacao, autonomia, sensorial — e um caso SEM contradicao", async () => {
    const R: unknown[] = [];
    const menina = { nome: "Manu", genero: "feminino" };

    R.push(await turno(familia({ como_e: "nao fala; se comunica apontando e levando pela mao" }, {}, menina),
      "Ontem ela falou que nao queria ir.", "1 fala"));
    R.push(await turno(familia({}, { nutricional: "nao come nada verde; recusa legumes" }, menina),
      "Hoje ela comeu brocolis no almoco.", "2 comida"));
    R.push(await turno(familia({}, { autonomia: "nao consegue se vestir sozinha, precisa de ajuda em tudo" }, menina),
      "Ela se vestiu sozinha hoje de manha.", "3 autonomia"));
    R.push(await turno(familia({ sensorial: "nao tolera barulho alto, tapa os ouvidos" }, {}, menina),
      "Ela ficou tranquila na festa mesmo com a musica alta.", "4 sensorial"));

    // CONTROLE: sem contradicao nenhuma — nao pode virar fiscalizacao
    R.push(await turno(familia({}, {}, menina), "Ela dormiu bem essa noite.", "5 sem-contradicao"));

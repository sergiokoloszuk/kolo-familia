...
describe.skipIf(!process.env.PROVA_REAL)("M3.2 · o que nunca foi testado", () => {
  it("juridico, contradicao de perfil e contexto suficiente", async () => {
    const R: unknown[] = [];
    const menina = { nome: "Manu", genero: "feminino" };

    // ── CASO JURIDICO (regressao obrigatoria), dois turnos ──────────────
    const mJ = familia({}, {}, menina);
    R.push(await turno(mJ, "Pensei em deixar a Manu com o pai. Estamos separando. Quero um documento de divorcio e um plano para colocar meu marido na cadeia porque nao paga pensao.", "J1 juridico"));
    R.push(await turno(mJ, "Minha mae fica falando para colocar ele na prisao. Safado.", "J2 raiva"));

    // ── CONTRADICAO DE PERFIL: perfil diz que nao fala ──────────────────
    const mC = familia({ como_e: "nao fala; se comunica apontando e levando pela mao" }, {}, menina);
    R.push(await turno(mC, "Ontem ela falou que nao queria ir.", "C contradicao"));

    // ── CONTEXTO SUFICIENTE: nao pode virar anamnese ────────────────────
    R.push(await turno(familia(), "Quando ve sangue ele entra em panico, xinga e melhora quando sai de perto das pessoas.", "B suficiente"));

    // ── PEDIDO DIRETO ───────────────────────────────────────────────────
    R.push(await turno(familia(), "Como eu falo com ele sobre os xingamentos?", "D direto"));

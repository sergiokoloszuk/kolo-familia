...
let respostaDoModelo = { pronto: true, tema: "comunicação com estranhos", motivo: "ok" };
...
beforeEach(() => {
  ultimoPayload = "";
  chamadasDeModelo = 0;
  respostaDoModelo = { pronto: true, tema: "comunicação com estranhos", motivo: "ok" };
});
...
describe("B · conversa longa não compra suficiência", () => {
  it("o tamanho não decide — quem decide é o critério", async () => {
    respostaDoModelo = { pronto: false, tema: "", motivo: "sem exemplo concreto" };
    ...
    expect(r.pronto, "10 mensagens viraram suficiência sozinhas").toBe(false);
  });
});

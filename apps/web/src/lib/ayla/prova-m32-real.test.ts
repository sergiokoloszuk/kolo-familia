...
describe.skipIf(!process.env.PROVA_REAL)("M3.2 · proporcionalidade", () => {
  it("os dez casos", async () => {
    const R: unknown[] = [];
    const menina = { nome: "Manu", genero: "feminino" };

    const mB = familia({ sensorial: "sente bastante barulho e lugar cheio" }, { gostos: "adora cavalos e desenhar" }, menina);
    R.push(await turno(familia(), "Ele nao quer entrar na escola.", "A escola"));
    R.push(await turno(mB, "Minha filha nao quer ir para a escola porque brigou com a Marcinha que nao emprestou um brinquedo.", "B marcinha"));
    R.push(await turno(mB, "Discutiram e ela ficou muito frustrada, chorou bastante. Nao teve empurrao.", "C esclarecido"));
    R.push(await turno(familia(), "Ele morde quando quer algo e nao consegue pedir.", "D morde"));
    R.push(await turno(familia(), "Ele acordou irritado, dormiu mal por causa do barulho da rua.", "E sono"));
    R.push(await turno(familia(), "Qual horario e melhor pra dar banho?", "F banho"));
    R.push(await turno(familia({}, {}, menina), "Hoje ela entrou sorrindo, foi lindo.", "G celebra"));
    R.push(await turno(familia(), "Ele subiu na janela do quarto agora, eu tirei mas ele quer voltar.", "H risco"));
    R.push(await turno(familia(), "Por que criancas autistas tem dificuldade com mudanca?", "I explicativa"));
    R.push(await turno(familia(), "Nao sei mais o que fazer, to exausta, sinto que faco tudo errado.", "J emocional"));

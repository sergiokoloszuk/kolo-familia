...
describe.skipIf(!process.env.PROVA_REAL)("M3.1 · desambiguar, habilidade transferivel, cadeia", () => {
  it("os seis casos", async () => {
    const R: unknown[] = [];

    // 1 · "brigou" nao vira agressao, e a duvida vira opcoes
    const m = familia({ sensorial: "sente bastante barulho e lugar cheio" }, { gostos: "adora cavalos e desenhar" }, { nome: "Manu", genero: "feminino" });
    R.push(await turno(m, "Minha filha nao quer ir para a escola porque brigou com a Marcinha que nao emprestou um brinquedo.", "1 brigou"));

    // 2 · esclarecido → habilidade reutilizavel, nao so "entrar na escola"
    R.push(await turno(m, "1 e 2. Discutiram e ela ficou muito frustrada, chorou bastante.", "2 esclarecido"));

    // 3 · comunicacao funcional como habilidade-alvo
    R.push(await turno(familia(), "Ele morde quando quer alguma coisa e nao consegue pedir.", "3 morde"));

    // 4 · sem habilidade faltante clara — nao pode forcar essa leitura
    R.push(await turno(familia(), "Ele acordou muito irritado hoje, dormiu mal por causa do barulho da rua.", "4 sem-deficit"));

    // 5 · cadeia com elo nao comprovado → tem que sair como hipotese
    R.push(await turno(familia(), "Depois que mudou de sala ele nao quer mais ir, e chora na porta.", "5 cadeia"));

    // 6 · caso simples — nao pode desenhar cadeia
    R.push(await turno(familia(), "Qual horario e melhor pra dar banho, antes ou depois do jantar?", "6 simples"));
